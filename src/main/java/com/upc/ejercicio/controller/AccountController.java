...
public class AccountController {
    ...
    private void validateAccount(Account account) {
        ...
        if (account.getNameCustomer().trim().length() > 31) {
            throw new ValidationException("El nombre del cliente no debe exceder los 31 caracteres");
        }
    ...
    private void existsAccountByNameCustomerAndNumberAccount(Account account) {
        if (_accountRepository.existsByNameCustomerAndNumberAccount(account.getNameCustomer().trim(), account.getNumberAccount().trim())) {
